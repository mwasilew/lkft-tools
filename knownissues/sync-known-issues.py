...
class SquadConnection(object):
    ...
    def put_object(self, endpoint, config):
        ...
        URL = urlunsplit((self.url_scheme, self.base_url, "api/%s/%s/" % (endpoint, object_id), None, None))
        logger.debug(URL)
        logger.debug(config)
...
class SquadKnownIssue(object):
    def __init__(self, config, squad_project):
        ...
        for item in config.get('environments'):
            if 'slug' in item.keys() and item['slug'] in squad_project.environment_slugs:
                self.environments.add(item['slug'])
            if 'architecture' in item.keys() and item['architecture'] in squad_project.environment_architectures.keys():
...
def main():
    ...
    for project_name, project in config_data.items():
        ...
        print(s.environment_architectures)
        for known_issue in s.known_issues:
            # create/uptade the issues in remote instance
            print(known_issue.title)
            print(known_issue.test_name)
            print(known_issue.environments)
            # for each project defined in the known_issue
            # get the environment IDs based on the environment name
            api_known_issue = s.connection.filter_object('knownissues', {'title': known_issue.title, 'test_name': known_issue.test_name})
            affected_environments = []
            for known_issue_project in known_issue.projects:
                group_name, project_name = known_issue_project.split('/', 1)
                api_project = s.connection.filter_object('projects', {'group__slug': group_name, 'slug': project_name})
                if api_project is None:
                    continue
                api_environments = s.connection.download_list('environments', {'project': api_project['id']})
                for api_env in api_environments:
                    if api_env['slug'] in known_issue.environments:
                        logger.debug("Adding env: %s to known issue: %s" % (api_env['slug'], known_issue.title))
